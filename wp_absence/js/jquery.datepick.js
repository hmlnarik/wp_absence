...
$(selector).datepick('setDate', [date1, date2, date3]) */
		...
		retrieveDate: function(elem, target) {
			...
			return ($.isEmptyObject(inst) ? null : this._normaliseDate(
                ...
                _invertSelectionOfDate: function(inst, date) {
                        var found = false;
                        for (var i = 0; i < inst.selectedDates.length; i++) {
                ...
                _addDateToSelection: function(inst, date) {
                        for (var i = 0; i < inst.selectedDates.length; i++) {
                                if (date.getTime() === inst.selectedDates[i].getTime()) {
                                        return;
                                }
                        ...
                        if (inst.selectedDates.length < inst.options.multiSelect) {
                                inst.selectedDates.push(date);
                        }
                },

                _removeDateFromSelection: function(inst, date) {
                        for (var i = 0; i < inst.selectedDates.length; i++) {
                                if (date.getTime() === inst.selectedDates[i].getTime()) {
                                        inst.selectedDates.splice(i, 1);
                                        return;
                ...
                _isSelected: function(inst, date) {
                        for (var i = 0; i < inst.selectedDates.length; i++) {
                                if (date.getTime() === inst.selectedDates[i].getTime()) {
                                        return true;
                                }
                ...
                _selectDatesBetween: function(elem, inst, dateA, dateB) {
                        var date = dateA < dateB ? dateA : dateB;
                        var maxSelDate = dateA >= dateB ? dateA : dateB;
                        var onDate = inst.options.onDate;

                        var minDate = (inst.pickingRange ? inst.selectedDates[0] : inst.get('minDate'));
			var maxDate = inst.get('maxDate');
                        
                        var selFn = (this._isSelected(inst, dateB) ? this._removeDateFromSelection : this._addDateToSelection);
                    
                        while (date <= maxSelDate) {
                                var dateInfo = (!$.isFunction(onDate) ? {} : onDate.apply(elem, [date]));
                                var selectable = this._isSelectable(elem, date, dateInfo.selectable, minDate, maxDate);
                                if (selectable) {
                                        selFn(inst, date);
                                }
		...
		selectDate: function(elem, target) {
			...
			if (!$.isEmptyObject(inst) && !this.isDisabled(elem)) {
				...
				if (inst.options.multiSelect) {
                                        var withShift = window.event && !!window.event.shiftKey; // typecast to boolean
                                        
                                        if (withShift) {
                                                this._selectDatesBetween(elem, inst, inst.prevDate, date);
                                        } else {
                                                this._invertSelectionOfDate(inst, date);
                                        }
				}
